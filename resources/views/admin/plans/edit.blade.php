            ...
            <div class="bg-white rounded-xl shadow-sm border p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="2" class="w-full px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('description', $plan->description) }}</textarea>
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border p-6">
                ...
                <div class="space-y-4">
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $plan->is_active) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500">
            ...
            <div class="bg-white rounded-xl shadow-sm border p-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="number" name="limits[qr_codes]" value="{{ old('limits.qr_codes', $plan->limits['qr_codes'] ?? 0) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Reviews / Month</label>
                        <input type="number" name="limits[reviews_per_month]" value="{{ old('limits.reviews_per_month', $plan->limits['reviews_per_month'] ?? 0) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Branches</label>
                        <input type="number" name="limits[branches]" value="{{ old('limits.branches', $plan->limits['branches'] ?? 0) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Employees</label>
                        <input type="number" name="limits[employees]" value="{{ old('limits.employees', $plan->limits['employees'] ?? 0) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">AI Credits</label>
                        <input type="number" name="limits[ai_credits]" value="{{ old('limits.ai_credits', $plan->limits['ai_credits'] ?? 0) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Analytics Days</label>
                        <input type="number" name="limits[analytics_days]" value="{{ old('limits.analytics_days', $plan->limits['analytics_days'] ?? 7) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                    </div>
                    
                    <div class="border-t pt-3 mt-3 space-y-3">
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">WhatsApp</span>
                            <input type="checkbox" name="limits.whatsapp" value="1" {{ old('limits.whatsapp', $plan->limits['whatsapp'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">SMS</span>
                            <input type="checkbox" name="limits.sms" value="1" {{ old('limits.sms', $plan->limits['sms'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">NFC Support</span>
                            <input type="checkbox" name="limits.nfc" value="1" {{ old('limits.nfc', $plan->limits['nfc'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">White Label</span>
                            <input type="checkbox" name="limits.white_label" value="1" {{ old('limits.white_label', $plan->limits['white_label'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">Custom Branding</span>
                            <input type="checkbox" name="limits.custom_branding" value="1" {{ old('limits.custom_branding', $plan->limits['custom_branding'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                        <label class="flex items-center justify-between cursor-pointer">
                            <span class="text-sm text-gray-700">Remove Branding</span>
                            <input type="checkbox" name="limits.remove_branding" value="1" {{ old('limits.remove_branding', $plan->limits['remove_branding'] ?? false) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 rounded">
                        </label>
                    </div>
                </div>
            </div>

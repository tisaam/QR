                ...
                <div class="plan-card">
                    ...
                    <div style="display:flex; flex-direction:column; gap:0.7rem;">
                        <div>
                            ...
                            <input type="number" name="limits[qr_codes]" value="{{ old('limits.qr_codes', $plan->limits['qr_codes'] ?? 0) }}">
                        </div>
                        <div>
                            <label>Reviews / Month</label>
                            <input type="number" name="limits[reviews_per_month]" value="{{ old('limits.reviews_per_month', $plan->limits['reviews_per_month'] ?? 0) }}">
                        </div>
                        <div>
                            <label>Branches</label>
                            <input type="number" name="limits[branches]" value="{{ old('limits.branches', $plan->limits['branches'] ?? 0) }}">
                        </div>
                        <div>
                            <label>Employees</label>
                            <input type="number" name="limits[employees]" value="{{ old('limits.employees', $plan->limits['employees'] ?? 0) }}">
                        </div>
                        <div>
                            <label>AI Credits</label>
                            <input type="number" name="limits[ai_credits]" value="{{ old('limits.ai_credits', $plan->limits['ai_credits'] ?? 0) }}">
                        </div>
                        <div>
                            <label>Analytics Days</label>
                            <input type="number" name="limits[analytics_days]" value="{{ old('limits.analytics_days', $plan->limits['analytics_days'] ?? 7) }}">
                        </div>
                        <div style="border-top:1px solid #e5e7eb; padding-top:0.8rem; display:flex; flex-direction:column; gap:0.45rem;">
                            <div class="check-row"><span>WhatsApp</span><input type="checkbox" name="limits.whatsapp" value="1" {{ old('limits.whatsapp', $plan->limits['whatsapp'] ?? false) ? 'checked' : '' }}></div>
                            ...
                            <div class="check-row"><span>NFC Support</span><input type="checkbox" name="limits.nfc" value="1" {{ old('limits.nfc', $plan->limits['nfc'] ?? false) ? 'checked' : '' }}></div>
                            <div class="check-row"><span>White Label</span><input type="checkbox" name="limits.white_label" value="1" {{ old('limits.white_label', $plan->limits['white_label'] ?? false) ? 'checked' : '' }}></div>
                            <div class="check-row"><span>Custom Branding</span><input type="checkbox" name="limits.custom_branding" value="1" {{ old('limits.custom_branding', $plan->limits['custom_branding'] ?? false) ? 'checked' : '' }}></div>
                            <div class="check-row"><span>Remove Branding</span><input type="checkbox" name="limits.remove_branding" value="1" {{ old('limits.remove_branding', $plan->limits['remove_branding'] ?? false) ? 'checked' : '' }}></div>
                        </div>
                    </div>
                </div>
